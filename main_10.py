...
if flags.dataset == "VLCS":
    if flags.seed == 5:
        indices = np.loadtxt('10v_5.txt',dtype='uint32')
    elif flags.seed == 6:
        indices = np.loadtxt('10v_6.txt',dtype='uint32')
    else:
        indices = np.loadtxt('10v_7.txt',dtype='uint32')
elif flags.dataset=="PACS":
    print('PACS')
    if flags.target_domain == 0:
        if flags.seed == 5:
            indices = np.loadtxt('p1005.txt',dtype='uint32')
        elif flags.seed == 6:
            indices = np.loadtxt('p1006.txt',dtype='uint32')
        elif flags.seed == 7:
            indices = np.loadtxt('p1007.txt',dtype='uint32')
        elif flags.seed ==8:
            indices = np.loadtxt('p1008.txt',dtype='uint32')
        else:
            indices = np.loadtxt('p1009.txt',dtype='uint32')
    elif flags.target_domain == 1:
        if flags.seed == 5:
            indices = np.loadtxt('p1015.txt',dtype='uint32')
        elif flags.seed == 6:
            indices = np.loadtxt('p1016.txt',dtype='uint32')
        elif flags.seed == 7:
            indices = np.loadtxt('p1017.txt',dtype='uint32')
        elif flags.seed ==8:
            indices = np.loadtxt('p1018.txt',dtype='uint32')
        else:
            indices = np.loadtxt('p1019.txt',dtype='uint32')
    elif flags.target_domain == 2:
        if flags.seed == 5:
            indices = np.loadtxt('p1025.txt',dtype='uint32')
        elif flags.seed == 6:
            indices = np.loadtxt('p1026.txt',dtype='uint32')
        elif flags.seed == 7:
            indices = np.loadtxt('p1027.txt',dtype='uint32')
        elif flags.seed ==8:
            indices = np.loadtxt('p1028.txt',dtype='uint32')
        else:
            indices = np.loadtxt('p1029.txt',dtype='uint32')
    else:
        if flags.seed == 5:
            indices = np.loadtxt('p1035.txt',dtype='uint32')
        elif flags.seed == 6:
            indices = np.loadtxt('p1036.txt',dtype='uint32')
        elif flags.seed == 7:
            indices = np.loadtxt('p1037.txt',dtype='uint32')
        elif flags.seed ==8:
            indices = np.loadtxt('p1038.txt',dtype='uint32')
        else:
            indices = np.loadtxt('p1039.txt',dtype='uint32')
...
ld_m = dict()
#Creation of a smaller training set.
indices = []
for i in range(flags.num_classes):
    for j in range(flags.n_domains):
        if(len(ld_dictionary['{}{}'.format(i,j)])//flags.nsamples == 0):
            ind = np.random.choice(ld_dictionary['{}{}'.format(i,j)],1)
        else:
            ind = np.random.choice(ld_dictionary['{}{}'.format(i,j)],len(ld_dictionary['{}{}'.format(i,j)])//flags.nsamples,replace=False)
        ind= np.sort(ind)
        ld_m['{}{}'.format(i,j)] = ind
        indices.extend(ind)
'''
