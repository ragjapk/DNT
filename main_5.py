...
if flags.dataset=="VLCS":
    indices = np.loadtxt('v/v{}{}{}.txt'.format(flags.nsamples,flags.target_domain,flags.seed),dtype='uint32')
elif flags.dataset=="PACS":
    print('PACS')
    indices = np.loadtxt('p/p{}{}{}.txt'.format(flags.nsamples,flags.target_domain,flags.seed),dtype='uint32')
elif flags.dataset=='OfficeHome':
    indices = np.loadtxt('o/o{}{}{}.txt'.format(flags.nsamples,flags.target_domain,flags.seed),dtype='uint32')
...
for epoch in range(flags.epochs):
    ...
    if vallossMeter.float()<best_val_loss and valcorrectMeter.float()>best_val_acc and epoch>flags.val_stop:
        ...
        torch.save(model.state_dict(),'saved/{}_{}_target{}_seed{}_nsample{}_alp{}.pt'.format(flags.dataset,flags.model,flags.target_domain,flags.seed,flags.nsamples,flags.alpha))
